...
export default function Landing({ weight }) {
  ...
  useEffect(() => {
    api.products({ sort: 'combined', health_weight: 1 - weight, page_size: 10 }).then(d => setProducts(d.results));
    api.stats().then(setStats);
    api.categories().then(c => setCats(c.slice(0, 12)));
  }, [weight]);
  ...
  return (
    <div style={{ padding: narrow ? '16px' : '20px 28px 40px', flex: 1, overflow: 'auto' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: narrow ? 'repeat(2, 1fr)' : '2fr 1fr 1fr 1fr', gap: 12, marginBottom: 18 }}>
        <div style={{ background: T.panel, borderRadius: 10, padding: 14, border: `1px solid ${T.line}`, display: 'flex', alignItems: 'center', gap: 10 }}>
          <input value={query} onChange={e => setQuery(e.target.value)}
            placeholder={`search ${stats ? stats.total_products.toLocaleString() : ''} products…`}
            onKeyDown={e => e.key === 'Enter' && nav(`/results?q=${query}`)}
      ...
      <div style={{ display: 'flex', gap: 6, marginBottom: 14, flexWrap: 'wrap' }}>
        {cats.map(c => (
          <button key={c.name} onClick={() => nav(`/results?category=${encodeURIComponent(c.name)}`)}
      ...
      <div style={{ background: T.panel, borderRadius: 10, border: `1px solid ${T.line}`, overflow: 'hidden' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: narrow ? '30px 1.5fr 70px 70px 50px' : '40px 50px 1.5fr 1fr 80px 80px 70px', gap: 10, padding: '8px 14px',
          ...
          <div>#</div>{!narrow && <div></div>}<div>Product</div>{!narrow && <div>Brand</div>}<div>Nutri</div><div>Eco</div><div>Score</div>
        </div>
        {products.map((p, i) => (
          <div key={p.code} onClick={() => nav(`/product/${p.code}`)}
            style={{ display: 'grid', gridTemplateColumns: narrow ? '30px 1.5fr 70px 70px 50px' : '40px 50px 1.5fr 1fr 80px 80px 70px', gap: 10, padding: '10px 14px',
            ...
            {!narrow && <ProductImage url={p.image_url} height={40} rounded={6} categories={p.categories} />}
